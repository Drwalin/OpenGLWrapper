...
namespace BasicMeshLoader {
	...
	template<typename T, uint32_t dim>
	void AssimpLoader::ConverterFloatPlain(T* dst, Value<dim> value) {
		for(int i=0; i<dim; ++i)
			dst[i] = value.v[i];
	}
	
	template<typename T, uint32_t dim>
	void AssimpLoader::ConverterFloatNormalized(T* dst, Value<dim> value) {
		ConvertFloatPlain(dst, Normalize(value));
	}
	
	template<typename T, uint32_t dim>
	void AssimpLoader::ConverterIntPlain(T* dst, Value<dim> value) {
		for(int i=0; i<dim; ++i)
			dst[i] = value.v[i]+0.5f;
	}
	
	template<typename T, T min, T max, uint32_t dim>
	void AssimpLoader::ConverterIntPlainClamp(T* dst, Value<dim> value) {
		for(int i=0; i<dim; ++i) {
	...
	void AssimpLoader::ConverterIntNormalized(T* dst, Value<dim> value) {
	...
	void AssimpLoader::ConverterUnsignedNormalized(T* dst, Value<dim> value) {
		value = Normalize(value);
		for(int i=0; i<dim; ++i)
	...
	inline Value<dim> AssimpLoader::Normalize(Value<dim> value) {
		if constexpr (dim > 1) {
			float sum = 0;
			for(int i=0; i<dim; ++i) {
				sum += value.v[i];
